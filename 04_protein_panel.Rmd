...
Get sample numbers:
```{r}
n_patients_CD_colon = npx_data_CD_colon |>
  dplyr::select(SampleID, group_tissue) |>
  distinct()
table(n_patients_CD_colon$group_tissue)
``` 
...
```{r}
write_tsv(t_res_CD_colon |> dplyr::select(-Threshold), file=file.path(params$artifact_dir, "SupplementaryTable10_CDcolon.tsv"))
``` 
...
Get sample numbers:
```{r}
n_patients_CD_ileum = npx_data_CD_ileum |>
  dplyr::select(SampleID, group_tissue) |>
  distinct()
table(n_patients_CD_ileum$group_tissue)
``` 
...
Write results table to file:
```{r}
write_tsv(t_res_CD_ileum |> dplyr::select(-Threshold), file=file.path(params$artifact_dir, "SuplementaryTable11_CDileum.tsv"))
``` 
...
Get sample numbers:
```{r}
n_patients_UC_colon = npx_data_UC_colon |>
  dplyr::select(SampleID, group_tissue) |>
  distinct()
table(n_patients_UC_colon$group_tissue)
``` 
...
Write results table to file:
```{r}
write_tsv(t_res_UC_colon |> dplyr::select(-Threshold), file=file.path(params$artifact_dir, "SupplementaryTable12_UCcolon.tsv"))
``` 
...
ggsave(file.path(params$artifact_dir, "Figure4B.pdf"), height = 5, width = 5)
```
...
ggplot(cor_results_combined_sig, aes(x = condition, y = reorder(Assay, correlation))) +
  ...
  scale_shape_manual(values = c("significant" = 8), name = expression(italic("Adj.P") < ".01")) +
  theme_cowplot() +
  labs(x = NULL,
       y = NULL,
       color = "Pearson's R",
...
ggsave(file.path(params$artifact_dir, "SupplementaryFigure5B.pdf"), height = 22, width = 6)
``` 
...
lgd_sig = Legend(pch = "*", type = "points", labels = expression(italic("Adj.P") < ".05"), labels_gp=gpar(fontsize=12))

draw(ht,
     annotation_legend_list = list(lgd_sig),
     merge_legend=TRUE)

pdf(file=file.path(params$artifact_dir, "SupplementaryFigure5C.pdf"), width=5, height=22)
draw(ht,
     annotation_legend_list = list(lgd_sig),
     merge_legend=TRUE)
dev.off()
