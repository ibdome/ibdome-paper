...
box::use(
   ...
   readr[write_tsv],
   rstatix[wilcox_test, add_xy_position],
   ggprism[add_pvalue]
)
...
ggsave(file.path(params$artifact_dir, "SupplementaryFigure3B.pdf"), width = 7, height =15)
```
...
Get sample numbers:
```{r}
n_patients_IBD = npx_data_IBDinf_noninf |>
  dplyr::select(SampleID, disease, inflammation_status) |>
  distinct()
table(n_patients_IBD$inflammation_status)
``` 
...
Get sample numbers:
```{r}
n_patients_CD = npx_data_CD |>
  dplyr::select(SampleID, disease, inflammation_status) |>
  distinct()
table(n_patients_CD$inflammation_status)
``` 
...
Get sample numbers:
```{r}
n_patients_UC = npx_data_UC |>
  dplyr::select(SampleID, disease, inflammation_status) |>
  distinct()
table(n_patients_UC$inflammation_status)
``` 
...
ggsave(file.path(params$artifact_dir, "SupplementaryFigure3E.pdf"), width = 7, height = 23)
```
...
```{r}
write.table(sort(olink_IBD_IPSS, decreasing=FALSE), file=file.path(params$artifact_dir, "SupplementaryTable7_IBD_IPSS.tsv"), sep="\t", row.names = FALSE, col.names = FALSE, quote=FALSE)
write.table(sort(olink_CD_IPSS, decreasing=FALSE), file=file.path(params$artifact_dir, "SupplementaryTable7_CD_IPSS.tsv"), sep="\t", row.names = FALSE, col.names = FALSE, quote=FALSE)
write.table(sort(olink_UC_IPSS, decreasing=FALSE), file=file.path(params$artifact_dir, "SupplementaryTable7_UC_IPSS.tsv"), sep="\t", row.names = FALSE, col.names = FALSE, quote=FALSE)
``` 
...
ggsave(file.path(params$artifact_dir, "SupplementaryFigure3C.pdf"), width = 5, height = 5)
```
...
Write to Supplementary Table 8:
```{r}
write_tsv(gsva_long_prot |> dplyr::select(sample_id, subject_id, date, IBD.IPSS, CD.IPSS, UC.IPSS), file=file.path(params$artifact_dir, "SupplementaryTable8_IPSSscores.tsv"))
``` 
...
Write to Supplementary Table 9:
```{r}
write_tsv(gsva_long |> dplyr::select(sample_id, subject_id, date, tissue, sampling_procedure, bMIS_IBD, bMIS_CD, bMIS_UC), file=file.path(params$artifact_dir, "SupplementaryTable9_bMISscores.tsv"))
``` 

### Plot bMIS-IBD scores with inflammation

```{r, fig.width=7, fig.height=5}
### Precompute statistics and change p-value style for plotting:
comps <- list(
  c("IBD_inflamed", "IBD_non_inflamed"),
  c("non-IBD", "IBD_non_inflamed"),
  c("IBD_inflamed", "non-IBD")
)

stat.test <- gsva_long |>
  filter(!is.na(group)) |>
  wilcox_test(bMIS_IBD ~ group, comparisons = comps, paired = FALSE) |>
  add_xy_position(x = "group") |>
  mutate(p_chr = formatC(p, format = "f", digits = 2), # numeric -> character with 2 decimals
         p_trim = ifelse(substr(p_chr, 1, 2) == "0.", substr(p_chr, 2, nchar(p_chr)), p_chr), # drop leading zero if present
         p_fmt = ifelse(p < .01, "< .01", p_trim),
         label = ifelse(p < .01, "italic(P) ~'<.01'", paste0("italic(P) ~ '=' ~ '", p_trim, "'"))
)
...
ggplot(gsva_long |> filter(! is.na(group)), aes(x=group, y=bMIS_IBD))+
  ...
  add_pvalue(
    ...
    parse = TRUE, 
    tip.length = 0,
    step.increase = 0.05, # adjust spacing
    label.size=4.5
)

ggsave(file.path(params$artifact_dir, "SupplementaryFigure3A.pdf"), width = 7, height = 5)
```
...
ggsave(file.path(params$artifact_dir, "Figure2D.pdf"), width = 12, height = 6)
```
