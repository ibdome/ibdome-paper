...
make_scatter = function(df, disease_filter, color_by, x, y, wjitter=NULL, hjitter=NULL) {
  p = df |> 
    ...
    theme_cowplot() + stat_cor(na.rm = TRUE, p.accuracy = .01) + ggtitle(disease_filter) + 
    geom_smooth(method="lm", fullrange=TRUE, se=FALSE, color="darkgray", show.legend = FALSE) + 
    ...
    theme(
      ...
      # Space between legend text and box
      legend.box.margin = margin(8, 12, 8, 12),
    )
  return(p)
}
```
...
pdf(file=file.path(params$artifact_dir, "SupplementaryFigure1A.pdf"), width=20, height=4)
draw(hm_patients,
     annotation_legend_list = list(lgd_sig),
     heatmap_legend_side = "bottom",
     annotation_legend_side = "bottom",
...
ggsave(file.path(params$artifact_dir, "Figure1A.pdf"), width = 6, height = 5)
``` 
...
p1+p2
ggsave(file.path(params$artifact_dir, "SupplementaryFigure1E.pdf"), width = 8, height = 4.5)
``` 
...
p1+ p2
ggsave(file.path(params$artifact_dir, "SupplementaryFigure1G.pdf"), width = 8, height = 4.5)
``` 
...
p1+ p2 
ggsave(file.path(params$artifact_dir, "SupplementaryFigure1F.pdf"), width = 8, height = 4.5)
```
...
# Get number of subjects per diseae
n_patients = coding_vars_NOD2_filtered |>
  select(subject_id, disease) |>
  distinct()

table(n_patients$disease)
``` 
...
pdf(file.path(params$artifact_dir, "Figure1D.pdf"), width = 15, height = 5)
lolliplot(mutation_gr,  # GRanges object with mutations
          features,
          type = "pie",
          legend = legends,
...
```{r}
write_tsv(resIHW, file=file.path(params$artifact_dir, "SupplementaryTable1.tsv"))
``` 
...
DCdata_long_inflammation <- ti_racle |> 
  ...
  left_join(rnaseq_metadata |> select('sample_id', 'group', 'disease', 'tissue_coarse'))
``` 

Get sample numbers:
```{r}
DCdata_long_inflammation |> 
  group_by(disease, tissue_coarse, group) |>  
  summarize(n_samples = n_distinct(sample_id)) |> 
  arrange(disease, tissue_coarse, group)
``` 
...
ggplot(DC_filtered, aes(x = group, y = fraction, fill = group)) +
  ...
  theme(
    ...
    legend.position = "bottom",
    legend.box.background = element_rect(color = "black", fill = NA), # add legend box
    legend.box.margin = margin(8, 12, 8, 12) # Space between legend text and box
  ) +
...
ggsave(file.path(params$artifact_dir, "SupplementaryFigure1G.pdf"), width = 19, height = 6)
``` 
