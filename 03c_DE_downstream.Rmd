...
(v1 | v2)
ggsave(file.path(params$artifact_dir, "SupplementaryFigure4B.pdf"), width = 10, height = 6)
``` 
...
# Save to file:
pdf(file=file.path(params$artifact_dir, "SupplementaryFigure4C.pdf"), width=20, height=10)
draw(hm,
...
p_IL12 <- ggplot(tpm_goi_box, aes(x = group_tissue, y = log_tpm)) +
  ...
  geom_signif(
    ...
    annotations = c(rep("italic('Adj.P')~'< .05'", 5), rep("ns", 19)),
  ...
  geom_boxplot(aes(group = group_tissue), 
               ...
               alpha = 0.3)  
p_IL12
ggsave(file.path(params$artifact_dir, "SupplementaryFigure4D.pdf"), height = 5.5, width = 9)
``` 
...
annotations_data <- data.frame(
  ...
  annotations = c(rep("italic('Adj.P')~'< .05'",8), rep("ns",16)),
  y_position = c(4.7, rep(3.2,3), rep(4, 4), 4, 3.2, 4.7, 4, 4.7, 4, 4.7, 4, 4.7, 3.2,  4.7, 3.2,  4.7, 3.2,  4.7, 3.2)
) %>%
  mutate(
